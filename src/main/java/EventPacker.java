...
public class EventPacker {
    ...
    /**
     * 2^K*M where K is the number of meetings with conflicts and M is the max size of the conflicting meetings
     *
     * See note below on possible improvement
     */
    public Agenda getMaxAttendees(int[][] meetings) {
        Set<Integer> dupIndexes = getDupIndexes(meetings);

        Agenda resolvedMeetings = getResolvedAgenda(meetings, dupIndexes);

        // Add back the non dupe meetings
        for(int i=0; i<meetings.length; i++) {
            if(dupIndexes.contains(i)) continue;
            resolvedMeetings.addMeeting(meetings[i]);
        }

        return resolvedMeetings;
    }

    private Agenda getResolvedAgenda(int[][] meetings, Set<Integer> dupIndexes) {
        // We are just making one group of duplicates even though they don't necessarily conflict with each other.
        // This should be optimised by breaking these up into duplicate groups.
        // ie something like if meetings 1,3 and 5 have conflicting attendees and meetings 2,4,6 have a different
        // set of conflicts we are unnecessarily backtracking across the entirety of 2,3,4,5,6 together.
        // Especially since backtracking here will be 2^N*M so if we can bring down the group size it will substantially reduce the time
        List<int[]> dupMeetings = new ArrayList<>();
        for(Integer dup : dupIndexes) {
            dupMeetings.add(meetings[dup]);
        }

        return selectMeetings(dupMeetings, 0, new Agenda());
    }

    /**
     * Backtracks across the entire meetings list to figure out which meetings are good or bad
     * O(2^N*M)
     */
    private Agenda selectMeetings(List<int[]> meetings, int index, Agenda currMeetings) {
        if(index == meetings.size()) {
            return currMeetings;
        }

        Agenda max = currMeetings;
        for(int i=index; i<meetings.size(); i++) {
            Agenda maxWithMeeting = null;
            int dupIndex = currMeetings.hasDuplicateAttendee(meetings.get(i));
            if (dupIndex != -1) {
                continue;
            }

            currMeetings.addMeeting(meetings.get(i));
            maxWithMeeting = selectMeetings(meetings, i + 1, currMeetings);
            max = max.attendeeCount > maxWithMeeting.attendeeCount ? max : new Agenda(maxWithMeeting);

            currMeetings.removeLastMeeting();
        }

        return max;
    }

    private Set<Integer> getDupIndexes(int[][] meetings) {
        Map<Integer, Integer> seenPos = new HashMap<>();
        Set<Integer> duplicates = new HashSet<>();
        for(int i = 0; i< meetings.length; i++) {
            for(Integer attendee : meetings[i]) {
                if (seenPos.containsKey(attendee)) {
                    // Add the previous one we saw since it's now duplicate
                    duplicates.add(seenPos.get(attendee));
                    duplicates.add(i);
                }
                seenPos.put(attendee, i);
            }
        }
        return duplicates;
    }
